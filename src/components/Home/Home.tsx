import { Link } from "react-router-dom";
import "./Home.css"
import Button from "../../common/Button/Button.tsx";
import Title from "../../common/Title/Title.tsx";

const Home = () => {
    return (
        <div className="divHome">
            <Title fontSize="6rem">Bookify</Title>
            <h2>Gestioná tus turnos de manera eficiente</h2>
            <div className="divButtonsHome">
                <Link className="linkButton" to={"/login/user"}>
                    <Button
                        iconSVG={
                            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="#FFFF" className="bi bi-person-fill" viewBox="0 0 16 16">
                                <path d="M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6" />
                            </svg>
                        }
                    >
                        Ingresar como usuario
                    </Button>
                </Link>
                <p className="pDescription">Para reservar turnos.</p>
                <Link className="linkButton" to={"/login/company"}>
                    <Button
                        iconSVG={
                            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="#FFFF" className="bi bi-building-fill" viewBox="0 0 16 16">
                                <path d="M3 0a1 1 0 0 0-1 1v14a1 1 0 0 0 1 1h3v-3.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5V16h3a1 1 0 0 0 1-1V1a1 1 0 0 0-1-1zm1 2.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5zm3 0a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5zm3.5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5M4 5.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5zM7.5 5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5m2.5.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5zM4.5 8h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5m2.5.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5zm3.5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5" />
                            </svg>
                        }
                    >
                        Ingresar como empresa
                    </Button>
                </Link>
                <p className="pDescription">Para gestionar los turnos de tus clientes.</p>
            </div>
        </div>
    );
}

export default Home;
