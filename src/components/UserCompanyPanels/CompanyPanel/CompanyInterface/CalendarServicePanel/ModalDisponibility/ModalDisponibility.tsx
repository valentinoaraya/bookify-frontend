...
const ModalDisponibility: React.FC<Props> = ({ isOpen, appointment, serviceId, setAvailableAppointments, setIsOpen, setAppointment }) => {
    ...
    const deleteAppointment = async (date: string) => {
                ...
                <div className="appointmentsDisponibilityContainer">
                    ...
                    {Array.from({ length: availableCount }).map((_, index) => (
                        <div key={`a-${index}`} className="disponibilitySlot available"
                            onClick={() => {
                                if (!isLoading) deleteAppointment(appointment?.datetime as string)
                            }}
                        >
                            <h3>Disponible</h3>
                        </div>
